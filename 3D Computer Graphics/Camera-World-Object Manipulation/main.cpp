...
void processFKeys(int key, int x, int y) {
	switch(key) {
		...
		case GLUT_KEY_F4: {
			...
			Globals::spin_angle = 5;
		    PointCloud bunny_cloud("pointclouds/bunny.xyz");
			//bunny_cloud.setMatrix(&cube.getMatrix());
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	case 'x':
		cout << "Move left" << endl;
		if(Globals::obj_id == 0) cube.move(-1, 0, 0);
	...
	case 'r':
		...
		if(Globals::obj_id == 0) cube.reset();	
	...
	case 's':
		...
		if(Globals::obj_id == 0) cube.scale(0.9);
		if(Globals::obj_id >= 3) Globals::cur_cloud.scale(0.9);
		break;
	case 'S':
		cout << "Scale up" << endl;
		if(Globals::obj_id == 0) cube.scale(1.1);
