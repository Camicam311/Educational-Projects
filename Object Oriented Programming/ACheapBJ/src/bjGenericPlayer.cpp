...
bjGenericPlayer::bjGenericPlayer(standardDeck& deckToUse) :
		referenceDeck(deckToUse) {
	double bet = 0.00;
	bjHand hand;
...
bjGenericPlayer::bjGenericPlayer(string player_name, standardDeck& deckToUse):
	referenceDeck(deckToUse), playerName(player_name)  {
	double bet = 0.00;
	bjHand hand;
	hands.push_back(hand);
	currentBets.push_back(bet);
...
bjGenericPlayer::bjGenericPlayer(standardDeck& deckToUse, bjHand hand) :
		referenceDeck(deckToUse) {
	double bet = 0.00;
	setHand(hand);
	currentBets.push_back(bet);
...
void bjGenericPlayer::split(bjHand &hand) {
	...
	if ((hand.getSize() == 2)
			&& (hand.getCard(0).getIntValue() == hand.getCard(1).getIntValue())
			&& (hands.size() <= 5)) {
		//Add second hand
		bjHand tempHand;
...
bjGenericPlayer& bjGenericPlayer::operator= (const bjGenericPlayer& rhs) {
	this->playerName = rhs.playerName;
	this->bankRoll = rhs.bankRoll;
	this->hands = rhs.hands;
	this->currentBets = rhs.currentBets;
	return *this;
}
