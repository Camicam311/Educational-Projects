...
class traced(object):
    #Change this to display __repr__ of the args
    def __init__(self,f):
            self.__nestlvl = 0
            self.__f=f
            self.__name__= f.__name__
    def __call__(self, *args, **dargs):
        arglst = []
        fxnstr = ("| "*self.__nestlvl) + ",- " + self.__name__ + "("
        for arg in args:
            arglst.append(str(arg) + ", ")
        for key in dargs:
            arglst.append(str(key)+"="+str(dargs[key]) + ", ")
        argstr = "".join(arglst)
        fxnstr += argstr.rstrip(', ') + ")"
        print fxnstr
        self.__nestlvl += 1
        rval = self.__f(*args, **dargs)
        self.__nestlvl -= 1
        print ("| "*self.__nestlvl) + "`- " + str(rval)
        return rval
